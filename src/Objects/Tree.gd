...
func _apply_variant():
	...
	if get_node_or_null("Sprite/Top"):
		$Sprite/Top.frame = topVariants[variant_clamped]

	if get_node_or_null("Sprite/Bottom"):
		$Sprite/Bottom.frame = bottomVariants[variant_clamped]
